          ...
          <Wrapper
            ...
            style={{
              ...
              ...(hasCard ? {
                ...
                paddingLeft: 16,
                borderLeft: node.pillColor
                  ? `2px solid ${node.pillColor.replace("1)", "0.45)")}`
                  : "2px solid var(--border)",
              }),
            }}
          >
            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="text-[9px] tracking-[0.2em] uppercase" style={{ color: node.pillColor ?? "var(--midtone)", opacity: hasCard ? 1 : 0.7 }}>
                {node.period}
              </span>
              {node.pill && (
                <span
                  ...
                  style={{
                    color: node.pillColor ?? "var(--midtone)",
                    borderColor: node.pillColor ? node.pillColor.replace("1)", "0.4)") : "var(--border)",
                    background: node.pillColor ? node.pillColor.replace("1)", "0.1)") : "transparent",
                    opacity: hasCard ? 1 : 0.7,
                  }}
                >
                  {node.pill}
                </span>
            ...
            {node.comingSoon && (
              <span className="text-[9px] tracking-[0.14em] uppercase mt-1" style={{ color: "var(--midtone)" }}>Coming Soon</span>
            )}
    ...
    return scrollYProgress.on("change", (v) => {
      ...
      WAVE_ANCHORS.forEach((anchor, i) => {
        ...
        if (inZone && !inZoneRef.current[i]) {
          setFlippedSet(p => { const next = new Set(p); if (next.has(i)) next.delete(i); else next.add(i); return next; });
        }
  ...
  return (
    <div ref={containerRef} className="relative w-full" style={{ height: SVG_H }}>
      ...
      {journeyNodes.map((node, i) => {
        ...
        const pc = node.pillColor;

        // Career nodes — unchanged left-border text style
        if (isCareer) {
          return (
            <div
              key={node.id}
            ...
            >
              <div
                style={{
                  ...
                  borderLeft: isRight ? (pc ? `2px solid ${pc.replace("1)", "0.45)")}` : "2px solid var(--border)") : "none",
                  borderRight: !isRight ? (pc ? `2px solid ${pc.replace("1)", "0.45)")}` : "2px solid var(--border)") : "none",
                }}
              >
                <div className={`flex items-center gap-1.5 ${isRight ? "" : "justify-end"}`}>
                  <span className="text-[9px] tracking-[0.2em] uppercase" style={{ color: pc ?? "var(--midtone)", opacity: 0.7 }}>
                    {node.period}
                  </span>
                  {node.pill && (
                    <span
                      className="text-[9px] tracking-[0.12em] uppercase px-1.5 py-0.5 border"
                      style={{
                        color: pc ?? "var(--midtone)",
                        borderColor: pc ? pc.replace("1)", "0.4)") : "var(--border)",
                        background: pc ? pc.replace("1)", "0.1)") : "transparent",
                        opacity: 0.7,
                      }}
                    >
                      {node.pill}
        ...
        const cardStyle: React.CSSProperties = {
          ...
          ...GLASS,
          ...(pc ? {
            background: pc.replace("1)", "0.1)"),
            border: `1px solid ${pc.replace("1)", "0.35)")}`,
          } : {}),
          ...
          >
            ...
            >
              ...
              >
                ...
                {/* Back face — details */}
                <div style={{ ...cardStyle, transform: "rotateY(180deg)", display: "flex", flexDirection: "column", gap: 6, textAlign: isRight ? "left" : "left" }}>
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <span className="text-[9px] tracking-[0.2em] uppercase" style={{ color: pc ?? "var(--midtone)" }}>
                      {node.period}
                    </span>
                    {node.pill && (
                      <span
                  ...
                  {node.comingSoon ? (
                    ...
                    <span className="text-[9px] tracking-[0.14em] uppercase" style={{ color: "var(--accent)" }}>Built with Claude Code</span>
                  ) : (
                    <span className="text-[9px] tracking-[0.14em] uppercase opacity-50 group-hover:opacity-100 transition-opacity duration-300" style={{ color: "var(--accent)" }}>
                      View case study ↗
                    </span>
                  )}
                </div>
