...
export default function Home() {
  ...
  return (
    <>
      ...
      {/* ── HERO: Split Screen ── */}
      <section className="relative min-h-[100dvh] flex flex-col md:flex-row overflow-hidden" style={{ zIndex: 1 }}>
        ...
        <motion.div
          ...
          className="relative w-full md:w-1/2 h-[42vh] md:h-auto flex-shrink-0"
        >
          <Image src="/nikki.jpg" alt="Niharika Mishra" fill priority style={{ objectFit: "cover", objectPosition: "center 15%", filter: "grayscale(1) contrast(1.1) brightness(1.02)" }} />
          {/* Accent bleed on right edge */}
          <div className="absolute inset-y-0 right-0 w-1" style={{ background: "var(--accent)" }} />
          <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 1.2, ease: EASE }} className="absolute top-8 left-6 text-[10px] tabular-nums tracking-[0.18em] text-white/40 select-none z-10">001</motion.span>
        </motion.div>
        ...
        <motion.div
          ...
          className="flex-1 flex flex-col justify-between px-8 sm:px-14 py-10 md:py-12 min-w-0"
          style={{ ...FONT }}
        >
          {/* Top row */}
          <div className="flex items-start justify-between">
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.9, ease: EASE }} className="text-[10px] tracking-[0.22em] uppercase text-[var(--midtone)]">
              Niharika Mishra · Experience Design · Capital One
            </motion.p>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5, delay: 0.9, ease: EASE }} className="flex flex-col items-end" style={{ lineHeight: 1 }}>
              <p className="font-semibold tabular-nums" style={{ fontSize: "clamp(18px, 2.2vw, 26px)", letterSpacing: "-0.02em", color: "var(--foreground)" }}>{String(new Date().getMonth() + 1).padStart(2, "0")}</p>
              <p className="font-semibold tabular-nums" style={{ fontSize: "clamp(18px, 2.2vw, 26px)", letterSpacing: "-0.02em", color: "var(--foreground)" }}>{String(new Date().getDate()).padStart(2, "0")}<span style={{ color: "var(--accent)" }}>.</span></p>
            </motion.div>
          </div>
          ...
          <div className="flex flex-col">
            {["I choose problems", "that matter to people."].map((line, i) => (
              <div key={i} style={{ overflow: "hidden" }}>
                <motion.p initial={{ y: "106%" }} animate={{ y: 0 }} transition={{ duration: 1, delay: 1.0 + i * 0.1, ease: EASE }} className="font-light text-[var(--foreground)]" style={{ fontSize: "clamp(26px, 3.8vw, 52px)", letterSpacing: "-0.02em", lineHeight: 1.15 }}>
          ...
          {/* Nav */}
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 1.5, ease: EASE }} className="flex flex-wrap items-center gap-8">
