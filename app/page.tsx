...
export default function Home() {
  ...
  return (
    <>
      ...
      <section className="relative min-h-[100dvh] flex flex-col items-center justify-center px-6 text-center overflow-hidden" style={{ ...FONT, zIndex: 1 }}>

        {/* Date — top right, absolute */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.6, ease: EASE }} className="absolute top-10 right-8 flex flex-col items-end" style={{ lineHeight: 1 }}>
          <p className="font-semibold tabular-nums" style={{ fontSize: "clamp(16px, 2vw, 22px)", letterSpacing: "-0.02em", color: "var(--foreground)" }}>{String(new Date().getMonth() + 1).padStart(2, "0")}</p>
          <p className="font-semibold tabular-nums" style={{ fontSize: "clamp(16px, 2vw, 22px)", letterSpacing: "-0.02em", color: "var(--foreground)" }}>{String(new Date().getDate()).padStart(2, "0")}<span style={{ color: "var(--accent)" }}>.</span></p>
        </motion.div>

        {/* Center stack */}
        <div className="flex flex-col items-center gap-8 max-w-2xl">
          <motion.p initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.75, ease: EASE }} className="text-[10px] tracking-[0.3em] uppercase text-[var(--midtone)]">
          ...
          <div className="flex flex-col items-center">
            {["I choose problems", "that matter to people."].map((line, i) => (
              <div key={i} style={{ overflow: "hidden" }}>
                <motion.p initial={{ y: "106%" }} animate={{ y: 0 }} transition={{ duration: 1.1, delay: 0.95 + i * 0.12, ease: EASE }} className="font-light text-[var(--foreground)]" style={{ fontSize: "clamp(24px, 3.6vw, 48px)", letterSpacing: "-0.025em", lineHeight: 1.2 }}>
          ...
          {/* Rule + nav */}
          <motion.div initial={{ opacity: 0, scaleX: 0 }} animate={{ opacity: 1, scaleX: 1 }} transition={{ duration: 0.8, delay: 1.4, ease: EASE }} style={{ transformOrigin: "center" }} className="w-full h-px bg-[var(--border)]" />

          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 1.6, ease: EASE }} className="flex items-center gap-8">
