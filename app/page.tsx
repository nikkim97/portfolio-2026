...
const HEADLINE_LINES = ["I choose", "problems", "that matter", "to people."];
...
export default function Home() {
  ...
  return (
    <>
      ...
      <section className="relative min-h-[100dvh] flex flex-col px-6 sm:px-14 py-10 overflow-hidden" style={{ ...FONT, zIndex: 1 }}>
        ...
        {/* Main headline — full width, massive */}
        <div className="flex-1 flex flex-col justify-center gap-0 mt-8">
          {HEADLINE_LINES.map((line, i) => (
            <div key={i} style={{ overflow: "hidden" }}>
              <motion.p
                initial={{ y: "110%" }}
                animate={{ y: 0 }}
                transition={{ duration: 1.1, delay: 0.85 + i * 0.09, ease: EASE }}
                className="font-light text-[var(--foreground)] leading-[0.92]"
                style={{ fontSize: "clamp(52px, 10vw, 130px)", letterSpacing: "-0.04em" }}
              >
                {line}
                {i === HEADLINE_LINES.length - 1 && <span style={{ color: "var(--accent)" }}></span>}
              </motion.p>
            </div>
          ))}
        </div>
        ...
        {/* Bottom row */}
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 1.5, ease: EASE }} className="flex items-end justify-between">
          <div className="hidden md:block relative" style={{ width: "clamp(120px, 14vw, 180px)", height: "clamp(120px, 14vw, 180px)", borderRadius: "50%", overflow: "hidden", filter: "grayscale(1) contrast(1.1)" }}>
            <Image src="/nikki.jpg" alt="Niharika Mishra" fill style={{ objectFit: "cover", objectPosition: "center 15%" }} />
          </div>
